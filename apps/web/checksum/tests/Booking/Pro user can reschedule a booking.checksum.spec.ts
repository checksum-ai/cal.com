...
test(
  defineChecksumTest(
    `Pro user can reschedule a booking @bug`,
    "PRO_RESCHEDULE_001"
  ),
  { annotation: [{ type: "bug", description: `test` }] },
  async ({ page, users, bookings }) => {
    let pro: any;
    let eventType: any;

    await checksumAI(
      "Create a pro user for testing rescheduling functionality",
      async () => {
    ...
    await checksumAI(
      "Create a booking for the pro user to reschedule",
      async () => {
    ...
    await checksumAI(
      "Login as the pro user to access booking management",
      async () => {
        await pro.apiLogin();
      }
    );
    ...
    await checksumAI(
      "Open the actions dropdown for the first booking",
      async () => {
        await page
    ...
    await checksumAI(
      "Click the reschedule option from the dropdown menu",
      async () => {
    ...
    await checksumAI(
      "Wait for the URL to contain the reschedule booking ID",
      async () => {
    ...
    await checksumAI(
      "Navigate to next month to find available time slots",
      async () => {
    ...
    await checksumAI(
      "Select the first available day in the calendar",
      async () => {
        await page
    ...
    await checksumAI(
      "Confirm the reschedule by clicking the confirm button",
      async () => {
