...
test(
  defineChecksumTest(`Book on column layout @bug`, "LAYOUT_COLUMN_001"),
  { annotation: [{ type: "bug", description: `test` }] },
  async ({ page, users }) => {
    let user: any;

    await checksumAI(
      "Create a user for testing column layout booking",
      async () => {
    ...
    await checksumAI(
      "Select the first available time slot in column view",
      async () => {
