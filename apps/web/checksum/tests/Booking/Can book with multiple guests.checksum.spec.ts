...
test(
  defineChecksumTest("Can book with multiple guests", "PRO_GUESTS_001"),
  {
    annotation: {
      type: "IntentionallyBroken",
      description: {
        change:
          "Use correct locator for adding guests '[data-testid=\"add-guests\"]' or any other way to click the correct button.",
      },
    },
  },
  async ({ page, users }) => {
    ...
    await checksumAI("Click the add guests button to add additional attendees", async () => {
      await page.locator('[data-testid="add-guests-button"]').click();
    });
