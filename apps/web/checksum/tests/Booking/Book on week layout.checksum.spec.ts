...
test(
  defineChecksumTest(`Book on week layout @bug`, "LAYOUT_WEEK_001"),
  { annotation: [{ type: "bug", description: `test` }] },
  async ({ page, users }) => {
    let user: any;

    await checksumAI(
      "Create a user for testing week layout booking",
      async () => {
    ...
    await checksumAI(
      "Click on the first event type to start booking",
      async () => {
    ...
    await checksumAI(
      "Click on an empty calendar cell in week view",
      async () => {
        await page
          .locator('[data-testid="calendar-empty-cell"]')
          .nth(0)
          .click();
      }
    );
    ...
    await checksumAI("Fill in the attendee email address", async () => {
      await page
        .locator('[name="email"]')
        .fill(`${randomString(4)}@example.com`);
    });
    ...
    await checksumAI(
      "Confirm the booking by clicking the confirm button",
      async () => {
