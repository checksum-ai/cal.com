...
test(
  defineChecksumTest(
    `Redirect with special characters in username should not throw 500 @bug`,
    "SPECIAL_CHAR_003"
  ),
  { annotation: [{ type: "bug", description: `test` }] },
  async ({ page, users, variableStore }) => {
    await checksumAI(
      "Create a user with special characters and single event type",
      async () => {
    ...
    await checksumAI("Navigate to user page", async () => {
      ...
      variableStore.response = await page.goto(
        `/${variableStore.benny.username}`
      );
    });

    await expect(
      variableStore.response?.status(),
      "Redirect should work without 500 error"
    ).not.toBe(500);
  }
);
