...
test(
  ...
  async ({ page, users, variableStore }) => {
    ...
    await checksumAI("Verify that member-1 cannot edit the out of office entry", async () => {
      const editButton = page.getByTestId(`ooo-edit-${variableStore.member1User?.username}`);
      await expect(editButton, "Edit button should not be visible for non-admin member").not.toBeVisible();
    });

    await checksumAI("Verify that member-1 cannot delete the out of office entry", async () => {
      const deleteButton = page.getByTestId(`ooo-delete-${variableStore.member1User?.username}`);
      await expect(
        deleteButton,
    ...
    await checksumAI("Verify that member-1 cannot add new entries", async () => {
      const addEntryButton = page.getByTestId("add_entry_ooo");
      await expect(
        addEntryButton,
